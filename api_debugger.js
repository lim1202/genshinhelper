...
config[0] = "" // uid
config[1] = "os_asia" // os_asia 亚服
config[2] = "" // cookie
// ========= ↑将生成的配置粘贴这以上↑ ========

let arg = (process.argv.length > 2) ? process.argv[2] : ""

if (arg === "data") {
    getDailyNote(config[1].startsWith("os"))
} else {
    const ds = generateDS()
    console.log("DS: " + ds)
}

function getDailyNote(oversea = true) {
    const https = require('https');

    const options = {
        hostname: oversea ? "bbs-api-os.hoyolab.com" : "api-takumi-record.mihoyo.com",
        port: 443,
        path: "/game_record/app/genshin/api/dailyNote?server=" + config[1] + "&role_id=" + config[0],
        method: 'GET',
        headers: generateHeaders(oversea)
    }
...
function generateHeaders(oversea = true) {
    const ds = generateDS(oversea)
    ...
    if (oversea) {
        headers = {
            ...
            "User-Agent": "Mozilla/5.0 (Linux; Android 12; Pixel 3) AppleWebKit/605.1.15 (KHTML, like Gecko) miHoYoBBSOversea/2.9.1",
...
function generateDS(oversea = true) {
    ...
    let sign = md5("salt=" + salt + "&t=" + timestamp + "&r=" + randomStr + "&b=&q=role_id=" + config[0] + "&server=" + config[1])
...
/**
 * 返回原神便笺信息(国际服 Android)
 *
 * @return {Promise<ResinResponse>} 便笺数据
 */
async function getDataOsAndroid() {
    let randomStr = randomIntFromInterval(100000, 200000)
    let timestamp = Math.floor(Date.now() / 1000)
    let sign = md5("salt=okr4obncj8bw5a65hbnn5oo6ixjc3l9w&t=" + timestamp + "&r=" + randomStr + "&b=&q=role_id=" + config[0] + "&server=" + config[1])
    let req = new Request("https://bbs-api-os.hoyolab.com/game_record/app/genshin/api/dailyNote?server=" + config[1] + "&role_id=" + config[0])
    req.method = "GET"
    req.headers = {
        "DS": timestamp + "," + randomStr + "," + sign,
        "x-rpc-client_type": "2",
